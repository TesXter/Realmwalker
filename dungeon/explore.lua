...
function Explore.clear_path_and_target()
...
function Explore.set_start_location_target()
...
local function reconstruct_path(came_from, current)
    ...
    for i = 2, #path - 1 do
        ...
        local angle_threshold = math.rad(10)
...
local function find_objective()
    local actors = actors_manager.get_all_actors()
    for _, actor in pairs(actors) do
        local name = actor:get_skin_name()
        if name:find("Altar01") then
            return actor, "altar"
        elseif name:find("NoBoss_LootChest") then
            return actor, "chest"
        end
    end
    return nil, nil
...
local interact_objective = false
...
local function reach_objective()
    ...
    local objective, objective_type = find_objective()
    if objective then
        local player_pos = get_player_position()
...
Explore.check_if_stuck = check_if_stuck

function Explore.set_custom_target(target)
    --console.print("Setting custom target.")
    target_position = target
end

function Explore.movement_spell_to_target(target)
...
function Explore.move_to_target()
    move_to_target()
end
